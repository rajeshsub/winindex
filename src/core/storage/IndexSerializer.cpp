...
bool IndexSerializer::Serialize(
...
bool IndexSerializer::Deserialize(
    const std::wstring& filePath,
    ...
    if (fileSize < sizeof(IndexFileHeader)) return false;

    f.seekg(0);
    IndexFileHeader hdr{};
    f.read(reinterpret_cast<char*>(&hdr), sizeof(hdr));

    if (hdr.magic != 0x58444957u) return false;
    ...
    if (Crc32(payload.data(), payloadSize) != hdr.crc32) return false;
    ...
    auto readU16 = [&]() -> uint16_t {
        uint16_t v{}; memcpy(&v, p, sizeof(v)); p += sizeof(v); return v;
    };
    auto readU32 = [&]() -> uint32_t {
        uint32_t v{}; memcpy(&v, p, sizeof(v)); p += sizeof(v); return v;
    };
    auto readU64 = [&]() -> uint64_t {
        uint64_t v{}; memcpy(&v, p, sizeof(v)); p += sizeof(v); return v;
    };
