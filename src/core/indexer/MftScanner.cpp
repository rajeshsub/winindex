...
bool MftScanner::IsExcluded(const std::wstring& path,
    ...
    for (const auto& excl : excludedPaths) {
        ...
        if (lower == exclLower) return true;
        if (exclLower.back() != L'\\') exclLower += L'\\';
        if (lower.starts_with(exclLower)) return true;
    }
    return false;
}

void MftScanner::Scan(const ScanOptions& options,
                       ScanCallback onFile,
...
bool MftScanner::ScanVolume(const std::wstring& root,
    ...
    while (!cancelToken.load(std::memory_order_relaxed)) {
        if (!DeviceIoControl(hVol, FSCTL_ENUM_USN_DATA, &med, sizeof(med),
        ...
        while (reinterpret_cast<BYTE*>(record) < buf.data() + bytesReturned) {
            if (record->FileNameLength > 0) {
                MftEntry entry;
                entry.name      = std::wstring(record->FileName,
                ...
                // USN records don't carry timestamps; we'll resolve via GetFileInformationByHandleEx
    ...
    std::function<std::wstring(DWORDLONG)> resolvePath = [&](DWORDLONG frn) -> std::wstring {
        ...
        if (it == entries.end()) return root;
        if (it->second.parentFrn == frn) return root; // root sentinel
    ...
    for (auto& [frn, entry] : entries) {
        ...
        if (IsExcluded(full, options.excludedPaths)) continue;
