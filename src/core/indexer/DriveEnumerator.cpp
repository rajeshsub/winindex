...
DriveFilesystem GetFilesystem(const std::wstring& root) {
    ...
    if (!GetVolumeInformationW(root.c_str(), nullptr, 0, nullptr, nullptr, nullptr,
                               fsName, static_cast<DWORD>(std::size(fsName)))) {
        return DriveFilesystem::Other;
    }
    if (wcscmp(fsName, L"NTFS") == 0) return DriveFilesystem::NTFS;
...
std::vector<DriveInfo> EnumerateLocalFixedDrives() {
    ...
    for (int i = 0; i < 26; ++i) {
        ...
        if (info.filesystem == DriveFilesystem::Other) continue;

        wchar_t label[MAX_PATH + 1] = {};
        GetVolumeInformationW(root, label, static_cast<DWORD>(std::size(label)),
