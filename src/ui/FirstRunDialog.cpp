...
INT_PTR CALLBACK FirstRunDialog::DlgProc(HWND hwnd, UINT msg, WPARAM wp, LPARAM lp) {
    ...
    switch (msg) {
        case WM_INITDIALOG:
            ...
            reinterpret_cast<FirstRunDialog*>(lp)->OnInit(hwnd);
            return TRUE;
        ...
        case WM_COMMAND:
            switch (LOWORD(wp)) {
                ...
                case IDC_MANUAL_ONLY:
                    // Toggle interval controls enabled state
                    if (self) {
...
void FirstRunDialog::OnInit(HWND hwnd) {
    SetWindowTextW(hwnd, L"winindex — First Run Setup");
    ...
    for (const auto& drive : m_drives) {
        ...
        SendMessageW(hDriveList, LB_SETSEL, isC ? TRUE : FALSE, idx);
    }
...
void FirstRunDialog::OnOk(HWND hwnd) {
    // Selected drives
    HWND hDriveList = GetDlgItem(hwnd, IDC_DRIVE_LIST);
