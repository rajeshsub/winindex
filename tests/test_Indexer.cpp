...
protected:
    ...
    void SetUp() override {
        ...
        indexer = std::make_unique<Indexer>(
            mftScanner, findScanner, usnMonitor, indexStore, settings);
    }
...
TEST_F(IndexerTest, FullScanWhenIndexInvalid) {
    ...
    EXPECT_CALL(*findScanner, Scan(_, _, _, _))
        .WillOnce(Invoke([](const ScanOptions&, ScanCallback cb,
                             ProgressCallback, const std::atomic<bool>&) {
            FileEntry e;
            e.name = L"test.txt";
            e.path = L"C:\\test.txt";
            e.size = 100;
...
TEST_F(IndexerTest, StatusCallbackFired) {
    ...
    indexer->SetStatusCallback([&](const IndexerStatus& s) {
